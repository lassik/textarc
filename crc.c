...
static const unsigned int crctab[256] = {0x00000000, 0x04c11db7, 0x09823b6e,
...
unsigned int
crc32_byte(unsigned int crc, unsigned int byte)
{
	return (crc << 8) ^ crctab[byte ^ (crc >> 24)];
}

unsigned int
crc32_done(unsigned int crc, unsigned long filesize)
{
	while (filesize) {
		crc = crc32_byte(crc, filesize & 255);
		filesize >>= 8;
	}
	return ~crc;
}
